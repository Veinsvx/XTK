...
var url = 'https://raw.githubusercontent.com/Veinsvx/XTK/main/QuestionBank.db';
var path = '/sdcard/veinsvx/QuestionBank.db';
var tkdown=CONFIG.get("TKDown","开启");
...
var isjs=CONFIG.get("radio1", "关闭");
var phonelock=CONFIG.get("phonelock","无锁");
var jspassword=CONFIG.get("jspassword");
var whether_mute=CONFIG.get("whether_mute","否");
var whether_froze=CONFIG.get("whether_froze","否");
...
var question_list = [];
var init_true = false;
var init_url = 'https://raw.githubusercontent.com/Veinsvx/XTK/main/question.txt';
var quesionpath='/sdcard/veinsvx/question.txt';
var file_tmp = false;
var tikus = '';
//#endregion
...
if(gest=="up"){
    toast("上滑解锁")
    swipe(dwc, dh/10*9, dwc, dh/10,100);
}
if(gest=="down"){
    swipe(dwc, dh/10, dwc, dh/10*9,100);
}
if(gest=="right"){
    swipe(dw/8, dhc, dw/8*7, dhc,100);
}
if(gest=="left"){
    swipe(dw/8*7, dhc, dw/8, dhc,100);
}//唤醒设备后上滑进入解锁界面

//滑动解锁屏幕
if(phonelock=="图案"){
    var arr = jspassword.split("-");
    var pwd = [];
    ...
    sleep(800);
    for (var y=0;y<pwd.length;y++){
        gesture(pwd[y][0],pwd[y][1],1500);
    }
}
if(phonelock=="密码")
{  
    for(var i = 0; i < jspassword.length; i++)
        {
...
if (shuangren == true || siren == true || 订阅 != 0 || stronger != 0 || tiaozhan) {
    ...
    if (tkdown=="开启") {
        threads.start(function () {
            ...
            //var qst = http.get(init_url).body.bytes();
            //files.writeBytes(quesionpath,qst);
        });
    }
    delay(2);
...
var vTime = CONFIG.get("time2", "10"); //每个小视频学习-5秒
...
function init() {
    ...
    if(tkdown=="开启"){
        threads.start(function() {
            try {
                var x = http.get('https://raw.githubusercontent.com/Veinsvx/XTK/main/replace.js').body.string();
                files.write('/sdcard/veinsvx/replace.js', x);
				r = require('/sdcard/veinsvx/replace.js');
            } catch (e) {}
            x = null;
        });
    }else{
    ...
    try {
        if(tkdown=="开启"){
			threads.start(startDownload);
			while(!file_tmp){
				toastLog('请等待四人/双人/挑战题库完成');
				delay(2);
			}     	
		}		
		file_tmp=null;
		tikus = files.read(quesionpath);
		toastLog('四人/双人/挑战题库加载完成');
        tikus = tikus.split('\n');
...
function startDownload() {
    toastLog('等待加载四人/双人/挑战题库!!!');
		try {
			var y = http.get(init_url).body.string();
			files.write(quesionpath, y);
			file_tmp=true;
			y=null;
			} 		
		catch (e) {
...
function zsyAnswer() {
    ...
    if (CONFIG.get("another", "2"))
        var x = CONFIG.get("another", "2") * 1;
    else
        var x = 0;
    while (x > 0) {
